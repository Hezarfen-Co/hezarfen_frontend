      ...
      <div class="space-y-5">
        ...
        <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <For each={links.filter((item) => !item.admin || auth.user()?.role === "admin")}>
            {(item) => (
              <Link to={item.to} class="flex min-h-20 items-center gap-3 rounded-lg border border-border bg-card p-4 hover:bg-muted/40 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring">
