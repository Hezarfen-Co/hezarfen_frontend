...
function MealsContent() {
  ...
  return (
    <div class="space-y-5">
      <PageHeader accent="violet" eyebrow={t("nav.group.classes")} title={t("meals.title")} description={t("meals.subtitle")} actions={<Show when={canManage()}><Button size="sm" class="min-w-30 rounded-lg" onClick={() => { setNewSlot(settings()?.meal_slots[0]?.name ?? ""); setShowCreate(true); }}><IconPlus class="h-4 w-4" />{t("meals.publish")}</Button></Show>} />
