            ...
            <For each={messages()}>
              {(message) => (
                <div class={message.role === "user" ? "ml-8 rounded-2xl rounded-br-sm bg-primary px-3.5 py-2.5 text-sm text-primary-foreground" : "mr-6 rounded-2xl rounded-bl-sm border border-border bg-card px-3.5 py-2.5 text-sm text-foreground shadow-sm"}>
                  <Show when={message.role === "assistant" && message.status === "pending"} fallback={<Show when={message.status === "failed" && !message.content} fallback={<CelebiMarkdown text={message.content} />}><p class="whitespace-pre-wrap leading-6">{failureMessage(message.error_code)}</p></Show>}>
