...
export function TabsList<T extends ValidComponent = "div">(
  ...
  return (
    <TabsPrimitive.List
      class={cn(
        "inline-flex max-w-full items-center gap-1 overflow-x-auto rounded-2xl border border-black/[0.06] bg-muted/45 p-1 text-muted-foreground shadow-sm dark:border-white/[0.08]",
        local.class,
      )}
      {...rest}
    />
...
export function TabsTrigger<T extends ValidComponent = "button">(
  ...
  return (
    <TabsPrimitive.Trigger
      class={cn(
        "inline-flex h-9 min-w-24 items-center justify-center whitespace-nowrap rounded-xl px-3 text-sm font-medium transition-all hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 data-[selected]:bg-background data-[selected]:text-foreground data-[selected]:shadow-sm",
        local.class,
      )}
      {...rest}
    />
