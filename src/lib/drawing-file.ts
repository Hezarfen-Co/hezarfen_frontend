import { parseScene, type DrawScene } from "./draw-stroke";
 ...
 * document.
 */
export function canvasToImageBlob(source: HTMLCanvasElement, mime: string): Promise<Blob | null> {
...
export async function sceneToPngFile(source: HTMLCanvasElement, scene: DrawScene, name: string): Promise<File> {
  const blob = await canvasToImageBlob(source, "image/png");
