...
- **Charts are allowed, on real data only.** The homepage follows the Knowvio-style
  ...
  split, and an Upcoming-deadlines table. Charts use the `Chart*` components in
  `src/components/ui/` (accent = `hsl(var(--primary))`, semantic tints for status).
  **Every chart/stat must be backed by a live API field** — no fabricated trends,
  streak counters, or `+%` delta badges the backend can't produce. Hide a panel when
  its data source is empty for the current role rather than faking it.
- **Layout order (top → bottom):**
  ...
  5. Optional role-scoped workspace links / recent list.
...
- **Deadlines table:** active / today / soon items only, semantic status badge, rows
  navigate to detail. Empty state is plain text, not a create CTA.
